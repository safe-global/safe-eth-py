...
class SafeCreationTx:
    def __init__(self, w3: Web3, owners: List[str], threshold: int, signature_s: int, master_copy: str,
        ...
        self.tx_pyethereum: Transaction = self._build_contract_creation_tx_with_valid_signature(self.tx_dict, self.s)
    ...
    def _get_initial_setup_safe_data(self, owners: List[str], threshold: int) -> bytes:
        return get_safe_contract(self.w3, self.master_copy).functions.setup(
            owners,
            threshold,
