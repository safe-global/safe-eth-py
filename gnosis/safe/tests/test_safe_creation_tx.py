...
class TestSafeCreationTx(SafeTestCaseMixin, TestCase):
    ...
    def test_safe_creation_tx_builder_with_not_enough_funds(self):
        ...
        with self.assertRaisesMessage(ValueError, "insufficient funds for gas"):
            w3.eth.send_raw_transaction(safe_creation_tx.tx_raw)

    def test_safe_creation_tx_builder_with_payment(self):
        logger.info(
            "Test Safe Proxy creation With Payment".center(LOG_TITLE_WIDTH, "-")
        )
        w3 = self.w3
    ...
    def test_safe_creation_tx_builder_with_token_payment(self):
        logger.info(
            "Test Safe Proxy creation With Gas Payment".center(LOG_TITLE_WIDTH, "-")
        )
        w3 = self.w3
        ...
        safe_creation_tx_2 = SafeCreationTx(
            w3=w3,
            owners=owners,
            threshold=threshold,
            signature_s=s,
        ...
        safe_creation_tx_3 = SafeCreationTx(
            w3=w3,
            owners=owners,
            threshold=threshold,
            signature_s=s,
        ...
        safe_creation_tx_4 = SafeCreationTx(
            w3=w3,
            owners=owners,
            threshold=threshold,
            signature_s=s,
        ...
        safe_creation_tx_5 = SafeCreationTx(
            w3=w3,
            owners=owners,
            threshold=threshold,
            signature_s=s,
    ...
    def test_safe_creation_tx_builder_with_fixed_cost(self):
        logger.info(
            "Test Safe Proxy creation With Fixed Cost".center(LOG_TITLE_WIDTH, "-")
        )
        w3 = self.w3
        ...
        self.assertEqual(tx_receipt.contractAddress, safe_address)
        self.assertEqual(
            w3.eth.get_balance(safe_address), safe_balance - fixed_creation_cost
        )
        self.assertLess(
            w3.eth.get_balance(deployer_address), safe_creation_tx.payment_ether
        )
        self.assertEqual(
            w3.eth.get_balance(funder_account.address),
            funder_balance + safe_creation_tx.payment,
    ...
    def test_safe_gas_with_multiple_owners(self):
        logger.info(
            "Test Safe Proxy creation gas with multiple owners".center(
                LOG_TITLE_WIDTH, "-"
            )
        )
        w3 = self.w3
        ...
        for i in range(2, number_of_accounts):
            ...
            logger.info(
                "Number of owners: %d - Gas estimated %d - Gas Used %d - Difference %d",
                len(owners),
