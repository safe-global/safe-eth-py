...
class Erc20Manager:
    ...
    def get_balances(
        ...
        response = self.http_session.post(
            self.ethereum_client.ethereum_node_url, json=queries
        )
        token_addresses_casted = cast(List[Union[Optional[str]]], [None]) + cast(
            List[Union[Optional[str]]], token_addresses
        )
        balances: List[BalanceDict] = []
        for token_address, data in zip(
            token_addresses_casted, sorted(response.json(), key=lambda x: x["id"])
        ):
            if "result" not in data:
                ...
                try:
                    if token_address:
                        balance = int(
                            self.w3.codec.decode_single(
                                "uint256", HexBytes(data["result"])
                            )
                        )  # Token
                    else:
...
class Erc721Manager:
    ...
    def get_balances(
        ...
        return [
            TokenBalance(token_address, 0 if balance is None else balance)
